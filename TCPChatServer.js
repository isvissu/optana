...
for(i=0;i<8;i++){
 ...
 var f = e^d;
 fcs = f;
 console.log( "a : "+a.toString(16)+" b : "+b.toString(16)+" c : "+c.toString(16)+" d : "+d.toString(16)+" e : "+e.toString(16)+" f : "+f.toString(16));
 }

787811010351608083526629800000010009e3e50d0a
 Start Bit : 7878
 Packet Length : 11
 Protocol Number : 01
 Terminal ID : 035160808352662980
 model Identification Code : 0000
 TimeZone and Language code : 0100
 Information Serial Number : 09
 Error Check : e3e5
 Stop Bit : 0d0a
 pdata = new Uint16Array(16);
 pdata[0] = 0x11;
 pdata[1] = 0x01;
 pdata[2] = 0x03;
 pdata[3] = 0x51;
...
//{{{
 function crct(pdata,nlength){
 var fcs = 0xffff;
 var CRCDic =new Uint16Array([
 0X0000,0X1189,0X2312,0X329B,0X4624,0X57AD,0X6536,0X74BF,
 0X8C48,0X9DC1,0XAF5A,0XBED3,0XCA6C,0XDBE5,0XE97E,0XF8F7,
 0X1081,0X0108,0X3393,0X221A,0X56A5,0X472C,0X75B7,0X643E,
 0X9CC9,0X8D40,0XBFDB,0XAE52,0XDAED,0XCB64,0XF9FF,0XE876,
 ...
 0X7BC7,0X6A4E,0X58D5,0X495C,0X3DE3,0X2C6A,0X1EF1,0X0F78
  ...
  var i = 0;
  while(nlength>0){
   console.log("i: "+i+" fcs: "+fcs.toString(16)+" pdata :"+pdata[i].toString(16) );
  fcs = (fcs>>8)^(CRCDic[(pdata[i]^(fcs))&0xff]);
  nlength--;
 i++;
  //console.log("i: "+i+" fcs: "+fcs.toString(16)+" pdata :"+pdata[i].toString(16) );
 }
 return fcs.toString(16);
 }
//}}}
...
//}}}
      ...
      crct(str.substring(4,38).toString(16),PacketLength.toString(16));
